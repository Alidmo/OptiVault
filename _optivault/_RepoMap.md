...
- [[src/ast/parser]] — exports: parseFile(filePath: string), ParseResult — deps: extractor
...
- [[src/compression/formatter.test]] — deps: formatter, parser
- [[src/compression/formatter]] — exports: formatVaultNote(parsed: ParseResult, notes?: string) — deps: parser
...
- [[src/mcp/query_graph.test]] — deps: server
- [[src/mcp/server.test]] — deps: server, parser, formatter
- [[src/mcp/server]] — exports: getTestFileCandidates(filename: string), normalizeGraphKey(key: string), buildDepGraph(repoMapContent: string), startMcpServer(vaultDir: string, sourceDir?: string), DepGraph — deps: parser, function-extractor, formatter, init
- [[src/vault/init.test]] — deps: init, parser, formatter
- [[src/vault/init]] — exports: processFile(filePath: string), readExistingConcepts(notePath: string), generateClaudeMd(dir: string, vaultDir: string), runInit(dir: string, outputDir: string), VaultRegistry — deps: parser, formatter, config
